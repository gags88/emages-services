{{-- Sticky Navigation --}}
<nav id="mainNav" class="fixed top-0 left-0 right-0 z-50 bg-white shadow-sm border-b border-surface-200"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-18">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2.5 group">
                ...
                <div>
                    <span class="text-lg font-display font-bold tracking-tight text-surface-900">Emages Soft</span>
                    <span class="text-lg font-display font-medium text-primary-600 tracking-tight">Services</span>
                </div>
            </a>

            {{-- Desktop Nav --}}
            <div class="hidden lg:flex items-center gap-1">
                <a href="{{ route('home') }}" class="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('home') ? 'text-primary-600 bg-primary-50 font-semibold' : 'text-surface-600 hover:text-surface-900 hover:bg-surface-100' }}">Home</a>
                <a href="{{ route('about') }}" class="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('about') ? 'text-primary-600 bg-primary-50 font-semibold' : 'text-surface-600 hover:text-surface-900 hover:bg-surface-100' }}">About</a>

                {{-- Services Dropdown --}}
                <div class="relative group" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                    <a href="{{ route('services') }}" class="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 flex items-center gap-1 {{ request()->routeIs('services*') ? 'text-primary-600 bg-primary-50 font-semibold' : 'text-surface-600 hover:text-surface-900 hover:bg-surface-100' }}">
                        Services
                        <i class="ph ph-caret-down text-xs transition-transform duration-200" :class="open ? 'rotate-180' : ''"></i>
                    </a>
                    <div x-show="open" x-cloak style="display: none;" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1"
                ...
                <a href="{{ route('portfolio') }}" class="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('portfolio') ? 'text-primary-600 bg-primary-50 font-semibold' : 'text-surface-600 hover:text-surface-900 hover:bg-surface-100' }}">Portfolio</a>
                <a href="{{ route('pricing') }}" class="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('pricing') ? 'text-primary-600 bg-primary-50 font-semibold' : 'text-surface-600 hover:text-surface-900 hover:bg-surface-100' }}">Pricing</a>
                {{-- Blog link hidden --}}
                <a href="{{ route('contact') }}" class="px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 {{ request()->routeIs('contact') ? 'text-primary-600 bg-primary-50 font-semibold' : 'text-surface-600 hover:text-surface-900 hover:bg-surface-100' }}">Contact</a>
            </div>
            ...
            <div class="flex items-center gap-4">
                ...
                {{-- Mobile Menu Button --}}
                <button @click="mobileOpen = !mobileOpen" class="lg:hidden relative w-10 h-10 flex items-center justify-center rounded-lg transition-colors bg-surface-100 hover:bg-surface-200">
                    <div class="w-5 h-4 flex flex-col justify-between">
                        <span class="w-full h-0.5 rounded-full transition-all duration-300 bg-surface-700" :class="mobileOpen ? 'rotate-45 translate-y-1.75' : ''"></span>
                        <span class="w-full h-0.5 rounded-full transition-all duration-300 bg-surface-700" :class="mobileOpen ? 'opacity-0' : ''"></span>
                        <span class="w-full h-0.5 rounded-full transition-all duration-300 bg-surface-700" :class="mobileOpen ? '-rotate-45 -translate-y-1.75' : ''"></span>
                    </div>
                </button>
            </div>
    ...
    {{-- Mobile Menu --}}
    <div x-show="mobileOpen" x-cloak style="display: none;" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2"
